#!/usr/bin/env python3
"""
alert_brainsight_v2.6.0.py
--------------------------
Real-time Brainsight drift monitor - version 2.6.0.

Adds (v2.5.0 -> v2.6.0):
  - Auto coil-swap following. Brainsight only writes `Crosshairs Position`
    rows for the coil currently selected in it, so a coil switch shows up as
    the driver name changing (e.g. `Coil A CT` -> `Coil B LCT`). The tracked
    driver now follows that switch. Enabled by default.
      * `--no-coil-follow` starts with it off (pick the driver from the menu
        and keep it pinned - classic behavior).
      * `set coil-follow on|off` toggles it live; turning it on jumps to the
        coil the file is currently on.
      * `set driver <n|name>` pins a driver manually and turns it OFF.
      * On a swap the old coil's last pose is discarded and, if triggering is
        on and the state was in range, STATE:RED is sent (stimulation stops at
        the swap). The new coil's first pose is then checked as usual, so
        STATE:GREEN follows as soon as it is on target. The target is kept -
        it is an MNI pose, valid for either coil.
  - Stale-pose timeout. If no crosshairs row arrives for the tracked driver
    for POINTER_STALE_SEC (30 s) - coil out of the camera's view - its last
    pose is dropped and drift checks pause, instead of judging an old pose
    indefinitely. No trigger is sent for this; the in/out-of-range state is
    kept, so the first transition after the coil reappears fires normally.
  ...
  - Participant ID. The study code for the session is entered HERE, on the Mac,
    ...
    participant it belongs to.
      ...
      * `set participant <id>` updates it live (e.g. to fix a typo); the new
        ...
        already written - reconnect to get a fresh row under the new id.
      * Entered on the Mac by design: the Windows receiver types `ss` into
        whatever window has focus, so text entry there could steal a trigger
        from QTrack mid-session.
...
-----
    ...
    python3 alert_brainsight_v2.6.0.py "path/to/Streamed Info.txt" --no-follow

    # Pin the crosshairs driver instead of following coil swaps:
    python3 alert_brainsight_v2.6.0.py "path/to/Streamed Info.txt" --no-coil-follow

    # With trigger to Windows receiver, labelling the session's sync rows:
    python3 alert_brainsight_v2.6.0.py "path/to/Streamed Info.txt" \\
        --trigger-to 192.168.1.20:5050 --token 1234 --participant SNBR-000
...
Terminal commands while running:
    ...
    set driver <n|name>           pin active driver (turns coil-follow OFF)
    set follow on|off             toggle auto-follow of the file's selection
    set coil-follow on|off        toggle following coil swaps in the file
    set trigger on|off            toggle sending SS triggers to Windows/QTrack
    set participant <id>          study code stamped on the time-sync log
...
PARTICIPANT_MAX_LEN = 64   # keeps SESSION: under TRIGGER_LINE_LIMIT
...
def sanitize_participant(value, limit=PARTICIPANT_MAX_LEN):
    ...
    pasted value carrying either would corrupt both. Mirrors
    trigger_app_AJ/common/protocol.sanitize_participant().
    """
    if not value:
        return ""
...
# Coil follow: when True the active driver tracks the driver named on the
# most recent Crosshairs Position row (i.e. the coil selected in Brainsight).
# A manual `set driver` turns it off.
coil_follow        = True
last_driver_seen   = None      # most-recent crosshairs driver in the file
# (old, new) driver switch for monitor_loop to apply, set when
# `set coil-follow on` jumps to the file's coil.
swap_pending       = None
...
# Participant / study code for this session. Sent to the Windows receiver on
# connect (and on change) so it can stamp the time-sync log rows with it.
participant_id     = ""
...
def scan_file(filepath):
    """Read the whole file once. Returns
    (targets, drivers, last_target_name, last_driver_name) where the last_*
    names are the most-recent Target Selection (MNI) and crosshairs driver."""
    ...
    return targets, drivers, last_target, last_driver
...
def print_list():
    ...
    for i, name in enumerate(drivers, 1):
        ...
        print(f"    {i}. {name}{marker}")
    cfollow_str = ("ON (tracking the coil selected in Brainsight)" if cfollow
                   else "OFF (pinned)")
    print(f"\n  Coil-follow: {cfollow_str}")
    print("\n  To change:  set target <number or name>  |  set driver <number or name>")
    print("              set follow on|off  |  set coil-follow on|off\n")
...
class TriggerSender:
    ...
    def __init__(self, host, port, token, participant="", on_log=None):
        self.host  = host
        self.port  = port
        self.token = token
    ...
    def set_participant(self, participant):
        """Update the study code and, if the link is up, tell Windows now.

        The receiver stamps the value it holds at the moment it writes a
        time-sync row, so an update here labels *subsequent* rows - rows
        already written keep the old value. Returns the sanitized id.
        """
        cleaned = sanitize_participant(participant)
        with self._sock_lock:
            self._participant = cleaned
            sock = self._sock
        if sock is not None:
            self._send_session(sock, cleaned)
        return cleaned

    def _send_session(self, sock, participant):
        """Best-effort SESSION: line. Never fatal - the link and the triggers
        matter more than the label."""
        try:
            sock.sendall(f"{PREFIX_SESSION}{participant}\n".encode("utf-8"))
            if participant:
                self._on_log(f"  [trigger] -> SESSION:{participant} "
                             f"(stamped on the Windows time-sync log)")
            else:
                self._on_log("  [trigger] -> SESSION: (blank - sync rows "
                             "will be unlabelled)")
            return True
        except OSError as exc:
            self._on_log(f"  [trigger] participant send failed: {exc}")
            return False
    ...
    def _connection_loop(self):
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                # Declare who this session is for BEFORE the time-sync, so the
                # very first row Windows logs already carries the participant.
                with self._sock_lock:
                    participant = self._participant
                self._send_session(sock, participant)
...
def input_thread():
    ...
    global coil_follow, swap_pending
    ...
    help_text = (
        ...
        "    set driver <n|name>           pin driver (turns coil-follow OFF)\n"
        "    set follow on|off             toggle auto-follow of file selection\n"
        "    set coil-follow on|off        toggle following coil swaps in file\n"
        "    set trigger on|off            toggle SS triggers to Windows/QTrack\n"
        "    set participant <id>          study code stamped on the sync log\n"
        "    set loc <mm>                  scalar linear threshold (all 3 axes)\n"
        "    set loc <x> <y> <z>           per-axis linear thresholds (mm)\n"
        "    set ang <rad>                 scalar angular threshold (all 3 axes)\n"
        "    set ang <x> <y> <z>           per-axis angular thresholds (rad)\n"
    ...
    while True:
        ...
        if cmd in ("quit", "q", "exit"):
            ...
            with lock:
                ...
                cfollow_str = "on" if coil_follow else "off"
                trig_str = "on" if triggers_enabled else "OFF (monitoring only)"
                who = participant_id or "(none)"
                print(
                    f"  [status]\n"
                    f"    subject : {who}\n"
            ...
            if key == "target":
                ...
                if chosen:
                    with lock:
                        ...
                        swap_pending       = None
                        reset_requested    = True
                    print(f"  [set]  driver pinned -> '{chosen}'  "
                          f"(coil-follow off, alert state reset)")

            elif key in ("coil-follow", "coilfollow", "coil_follow"):
                v = val_str.strip().lower()
                if v in ("on", "true", "1", "yes"):
                    with lock:
                        coil_follow = True
                        switched = None
                        if (last_driver_seen is not None
                                and active_driver_name != last_driver_seen):
                            # Same path as a swap seen in the file (monitor
                            # loop drops the old pose and stops stimulation).
                            swap_pending = (active_driver_name, last_driver_seen)
                            switched = last_driver_seen
                    print("  [set]  coil-follow ON - tracking the coil selected "
                          "in Brainsight")
                    if switched:
                        print(f"  [set]  switching to '{switched}'")
                elif v in ("off", "false", "0", "no"):
                    with lock:
                        coil_follow = False
                    print("  [set]  coil-follow OFF - driver pinned manually")
                else:
                    print("  [!] usage: set coil-follow on|off")
            ...
            elif key in ("participant", "subject"):
                cleaned = sanitize_participant(val_str)
                if not cleaned:
                    print("  [!] usage: set participant <study code>")
                else:
                    with lock:
                        participant_id = cleaned
                    if trigger_sender is not None:
                        trigger_sender.set_participant(cleaned)
                        print(f"  [set]  participant -> '{cleaned}'  (Windows "
                              f"will stamp it on time-sync rows from now on; "
                              f"rows already written keep the old id)")
                    else:
                        print(f"  [set]  participant -> '{cleaned}'  "
                              f"(no --trigger-to; nothing to log it to)")
...
def monitor_loop(filepath):
    global reset_requested
    global active_target_name, active_target, active_driver_name
    global last_selected_name, last_driver_seen, swap_pending
    ...
    while True:
        ...
        batch_pointers = {}   # driver -> latest MNI crosshairs pose this poll
            ...
            try:
                ...
                for raw in new_lines:
                    ...
                    if row_type == "Crosshairs Position":
                        ...
                        if not driver:
                            continue
                        # Brainsight only writes rows for the coil selected in
                        # it, so the name alone says which coil is in use.
                        batch_last_driver = driver
                        with lock:
                            if driver not in all_drivers:
                                all_drivers.append(driver)
                                newly_seen_drivers.append(driver)
                ...
                if batch_last_driver is not None:
                    with lock:
                        last_driver_seen = batch_last_driver
                        if active_driver_name is None:
                            # First driver ever: take the file's current coil
                            # when following, else the first one seen.
                            adopted = (newly_seen_drivers[0]
                                       if newly_seen_drivers and not coil_follow
                                       else batch_last_driver)
                            active_driver_name = adopted
                            reset_requested    = True
                        elif (coil_follow
                                and active_driver_name != batch_last_driver):
                            swap_pending = (active_driver_name, batch_last_driver)
                if adopted is not None:
                    print(f"[{ts()}]  + driver '{adopted}' auto-selected")

                with lock:
                    swap_to = swap_pending[1] if swap_pending else None
                for name in newly_seen_drivers:
                    if name not in (adopted, swap_to):
                        print(f"[{ts()}]  + discovered driver '{name}' "
                              f"(use 'set driver' to pin it)")

            except OSError as e:
                print(f"[{ts()}]  Read error: {e}")

        # ── Apply a coil swap (from the file or `set coil-follow on`) ────────
        with lock:
            swap = swap_pending
            swap_pending = None
            if swap is not None:
                active_driver_name = swap[1]
            swap_triggers = triggers_enabled
        if swap is not None:
            old, new = swap
            print(f"[{ts()}]  [SWAP] coil switched in Brainsight: "
                  f"'{old}' -> '{new}' - now tracking '{new}'")
            last_pointer = None
            pointer_lost = False
            if not in_exceedance:
                # Stimulation stops at the swap; the new coil's first
                # in-range pose sends GREEN through the normal transition.
                in_exceedance = True
                checks_over   = 1
                if trigger_sender is not None and swap_triggers:
                    trigger_sender.send_state(STATE_RED)
                print(f"[{ts()}]  [SWAP] stimulation stopped until '{new}' "
                      f"is on target")
            else:
                print(f"[{ts()}]  [SWAP] already out of range - stays stopped "
                      f"until '{new}' is on target")
        ...
        if fresh is not None:
            ...
            if pointer_lost:
                pointer_lost = False
                print(f"[{ts()}]  [FOUND] '{cur_driver}' visible again - "
                      f"drift checks resumed")
        elif (last_pointer is not None
                and now - last_pointer_at > POINTER_STALE_SEC):
            # Coil out of view: stop judging an old pose. No trigger is sent;
            # in_exceedance is kept so the next real transition fires.
            last_pointer = None
            pointer_lost = True
            print(f"[{ts()}]  [LOST] '{cur_driver}' not visible - no crosshairs "
                  f"data for {POINTER_STALE_SEC:.0f} s; drift checks paused "
                  f"(no trigger sent)")
        ...
        if cur_target is None or cur_driver is None or last_pointer is None:
            if now - last_status_time >= STATUS_INTERVAL:
                ...
                if cur_target is None:
                    ...
                    print(f"[{ts()}]  '{cur_driver}' still not visible - "
                          f"drift checks paused")
                else:
...
def main():
    ...
    global active_target_name, active_target, active_driver_name
    global auto_follow, last_selected_name
    global coil_follow, last_driver_seen
    global triggers_enabled
    global participant_id
    ...
    parser.add_argument("--no-coil-follow", dest="coil_follow",
                        action="store_false",
                        help="Disable coil-swap following: pick the crosshairs "
                             "driver from the menu and keep it pinned (classic "
                             "v2.5.0 behavior). Coil-follow is ON by default.")
    parser.set_defaults(coil_follow=True)
    ...
    participant = sanitize_participant(args.participant)

    with lock:
        thr_loc = [args.loc] * 3
        thr_ang = [args.ang] * 3
        auto_follow = args.follow
        coil_follow = args.coil_follow
    ...
    print(f"  Participant: {participant or '(none - sync rows will be unlabelled)'}")
    ...
    print(f"  Coil-follow: {'on' if args.coil_follow else 'off'}")
    ...
    if args.trigger_to is None and participant:
        print("  [i] --participant provided but no --trigger-to; there is no "
              "time-sync log to stamp it on.")
    ...
    (initial_targets, initial_drivers,
     initial_last_target, initial_last_driver) = scan_file(filepath)
    with lock:
        last_selected_name = initial_last_target
        last_driver_seen   = initial_last_driver
    ...
    if args.coil_follow:
        # Coil-follow: adopt the coil the file is currently on; the monitor
        # loop keeps switching as Brainsight's coil selection changes.
        if initial_last_driver is not None:
            with lock:
                active_driver_name = initial_last_driver
            print(f"  Coil-follow ON - tracking the coil currently selected "
                  f"in Brainsight: '{initial_last_driver}'")
            print(f"  (use 'set driver <n|name>' to pin one, or "
                  f"'set coil-follow off')")
        else:
            print("  Coil-follow ON - no Crosshairs Position rows yet; "
                  "will track the coil that appears.")
    elif initial_drivers:
